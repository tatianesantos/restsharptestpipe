...
namespace RestSharpNetCoreTemplate.Helpers
{
    public class ExtentReportHelpers
    {
        ...
        public static void AddTestInfo(string url, string requestService, string method, IDictionary<string, string> headers, IDictionary<string, string> cookies, IDictionary<string, string> parameters, object jsonBody, bool httpBasicAuthenticator, bool ntlmAuthenticator, Task<IRestResponse> response)
        {
            ...
            foreach (var responseHeader in response.Result.Headers)
            {
            ...
            if (jsonBody != null)
            {
                string jsonBodySerializer = jsonSerializer.Serialize(jsonBody);
                node.Log(Status.Info, "<pre>" + "<b>JSON BODY: </b>" + "\n" + jsonBodySerializer + "</pre>");
            }
            ...
            node.Log(Status.Info, "<pre>" + "<b>STATUS CODE: </b>" + numericStatusCode + " - " + response.Result.StatusCode.ToString() + "</pre>");
            node.Log(Status.Info, "<pre>" + "<b>RESPONSE HEADERS: </b>" + allResponseHeaders + "</pre>");
            node.Log(Status.Info, "<pre>" + "<b>PAYLOAD: </b>" + "\n" + GeneralHelpers.FormatJson(response.Result.Content) + "</pre>");
        }
